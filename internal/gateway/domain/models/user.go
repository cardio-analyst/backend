...
func (u User) Validate(validatePassword bool) error {
	...
	if err != nil {
		...
		if validationError, found := validationErrors["firstName"]; found {
			return fmt.Errorf("%w: %v", errors2.ErrInvalidFirstName, validationError)
		}
		if validationError, found := validationErrors["lastName"]; found {
			return fmt.Errorf("%w: %v", errors2.ErrInvalidLastName, validationError)
		}
		if validationError, found := validationErrors["region"]; found {
			return fmt.Errorf("%w: %v", errors2.ErrInvalidRegion, validationError)
		}
		if validationError, found := validationErrors["birthDate"]; found {
			return fmt.Errorf("%w: %v", errors2.ErrInvalidBirthDate, validationError)
		}
		if validationError, found := validationErrors["login"]; found {
			return fmt.Errorf("%w: %v", errors2.ErrInvalidLogin, validationError)
		}
		if validationError, found := validationErrors["email"]; found {
			return fmt.Errorf("%w: %v", errors2.ErrInvalidEmail, validationError)
		}
		if validationError, found := validationErrors["password"]; found {
			return fmt.Errorf("%w: %v", errors2.ErrInvalidPassword, validationError)
		}

		return errors2.ErrInvalidUserData
	}
	return nil
}
