...
func (d ScoreData) Validate(params ValidationOptionsScore) error {
	...
	if err != nil {
		...
		if validationError, found := validationErrors["gender"]; found {
			return fmt.Errorf("%w: %v", errors2.ErrInvalidGender, validationError)
		}
		if validationError, found := validationErrors["sbpLevel"]; found {
			return fmt.Errorf("%w: %v", errors2.ErrInvalidSBPLevel, validationError)
		}
		if validationError, found := validationErrors["totalCholesterolLevel"]; found {
			return fmt.Errorf("%w: %v", errors2.ErrInvalidTotalCholesterolLevel, validationError)
		}
		if validationError, found := validationErrors["age"]; found {
			return fmt.Errorf("%w: %v", errors2.ErrInvalidAge, validationError)
		}

		return errors2.ErrInvalidScoreData
	}
	return nil
}
